...
class MainPage implements WebPage {
  ...
  public void processPostRequest(HTTPRequest request, HTMLNode contentNode) {
    ...
    if (addURI != null && addURI.length() != 0) {
      try {
        ...
        pageMaker.getInfobox("infobox infobox-error", "Error adding URI", contentNode).addChild("#",
            e.getMessage());
  ...
  public void writeContent(HTTPRequest request, HTMLNode contentNode) {
    ...
    statusContent.addChild("#",
        "Running Request: " + runningFetch.size() + "/" + config.getMaxParallelRequests());
    ...
    statusContent.addChild("br");
    statusContent.addChild("#", "Last Activity: " + ((lastRequestFinishedAt == 0) ? "never"
        : (TimeUtil.formatTime(System.currentTimeMillis() - lastRequestFinishedAt) + " ago")));
    statusContent.addChild("br");
    statusContent.addChild("#", "Time stalled: " + TimeUtil.formatTime(tStalled));
    statusContent.addChild("br");
    statusContent.addChild("#", "Time not stalled: " + TimeUtil.formatTime(tNotStalled) + " ("
        + (tNotStalled * 100.0) / (tStalled + tNotStalled) + "%)");
    FreenetURI uri = spider.getURI();
    statusContent.addChild("br");
    if (uri != null && uri.getSuggestedEdition() >= 0) {
      statusContent.addChild("#", "Key: ");
      statusContent.addChild(HTMLNode.link("/" + uri.toString(false, false))).addChild("#",
          uri.toShortString());
    } else if (uri != null) {
    ...
    addForm.addChild("input", new String[] {"name", "style"},
        new String[] {"addURI", "width: 20em;"});
    ...
    if (runningFetch.isEmpty()) {
      ...
      HTMLNode list =
          runningContent.addChild("ol", "style", "overflow: auto; white-space: nowrap;");
