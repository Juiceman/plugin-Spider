...
class ConfigPage implements WebPage {
  ...
  public synchronized void processPostRequest(HTTPRequest request, HTMLNode contentNode) {
    ...
    if (request.isPartSet("maxParallelRequestsWorking")) {
      int v =
          request.getIntPart("maxParallelRequestsWorking", config.getMaxParallelRequestsWorking());
      config.setMaxParallelRequestsWorking(v);
    }
    if (request.isPartSet("maxParallelRequestsNonWorking")) {
      int v = request.getIntPart("maxParallelRequestsNonWorking",
  ...
  public void writeContent(HTTPRequest request, HTMLNode contentNode) {
    ...
    addConfig(spiderConfig, //
        "Max Parallel Requests (Working)",
        "Maximum number of parallel requests if we are in the working period.", //
        "maxParallelRequestsWorking", //
        new String[] {"0", "1", "2", "5", "10", "15", "25", "50", "75", "100", "125", "150", "200",
            "250", "500", "1000"}, //
        Integer.toString(config.getMaxParallelRequestsWorking()));
    addConfig(spiderConfig, //
        "Max Parallel Requests (Non-Working)",
        "Maximum number of parallel requests if we are not in the working period.", //
    ...
    addConfig(indexConfig, //
        "Buffer size for new format indexes (MB)",
        "In-memory buffer size for new format indexes. Probably a good idea for this to be fairly big, although we will use more memory than the limit given.", //
        "newFormatBufferSize", //
