...
/**
 ** A {@link TermEntry} that associates a subject term with a final target {@link FreenetURI} that
 * satisfies the term.
 ** 
...
public class TermPageEntry extends TermEntry {
  ...
  /**
   ** Map from positions in the text to a fragment of text around where it occurs. Only non-null if
   * we have the fragments of text (we may have positions but not details), to save memory.
   */
   ...
   ** @param p Map of positions (where the term appears) to context (fragment surrounding it).
   */
  ...
  /**
   * Get the positions to fragments map. If we don't have fragments, create this from the positions
   * list.
   */
