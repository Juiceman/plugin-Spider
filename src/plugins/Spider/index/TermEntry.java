 ...
 **
 ** Subclasses '''must''' override the following methods to use information specific to that
 * subclass:
 **
 ** * {@link #entryType()} * {@link #compareTo(TermEntry)} * {@link #equals(Object)} *
 * {@link #hashCode()}
 **
 ** @author infinity0
 */
abstract public class TermEntry implements Comparable<TermEntry> {
  ...
  /**
   ** Relevance rating. Must be in the half-closed interval (0,1]. a relevance of 0 indicates that
   * the relevance is unset
   */
  public float rel;
   ...
   **
   ** This implementation sorts by order of descending relevance, then by order of ascending
   * {@link #entryType()}.
   **
   ** Subclasses '''must overridde this method'' to also use information specific to the subclass.
   **
   ...
   ** {@inheritDoc}
   **
   ** This implementation tests whether the run-time classes of the argument is identical to the
   * run-time class of this object. If they are, then it tests the relevance and subject fields.
   **
   ** Subclasses '''must overridde this method'' to also use information specific to the subclass.
   */
  @Override
   ...
   **
   ** Subclasses '''must overridde this method'' to use information specific to the subclass.
   */
  ...
  /*
   ** Calculates an accumulated score to sort the entry by, using the formula relevance^3 * quality;
   * in other words, relevance is (for sorting results) 3 times as important as quality. For
   * example, for (rel, qual) we have (0.85, 0.95) < (1.0, 0.6) < (0.85, 1.00) / public float
   * score() { if (score_ == null) { score_ = new Float(rel * rel* rel * qual); } return score_; }
   */
