...
/**
 * Buffer which stores TermPageEntrys as they are found by the Spider. When the buffer's estimated
 * size gets up to bufferMax, the buffer is serialized into a Bucket and sent to the Library
 * 
 *
 * @author MikeB
 */
public class LibraryBuffer implements FredPluginTalker {
  ...
  /**
   * Takes a TermPageEntry and either returns a TPE of the same term & page from the buffer or adds
   * the TPE to the buffer and returns it.
   *
   * @param newTPE
   * @return
   */
  ...
  /**
   * Emptys the buffer into a bucket and sends it to the Library plugin with the command
   * "pushBuffer"
   *
   * FIXME : I think there is something wrong with the way it writes to the bucket, I may be using
   * the wrong kind of buffer
   */
  private void sendBuffer() {
    long tStart = System.currentTimeMillis();
    try {
      Logger.normal(this,
          "Sending buffer of estimated size " + bufferUsageEstimate + " bytes to Library");
    ...
    // Robustness: Send SAVE_FILE *after* sending new data, because *it is already on disk*, whereas
    // the new data is not.
    if (SAVE_FILE.exists()) {
  ...
  public void terminate() {
    ...
    try {
      ...
      totalPagesIndexed = -1;
      // FIXME I don't understand why this (ClassNotFoundException) happens, we have not closed the
      // class loader yet.
      System.err
          .println("Failed to determine page size during writing data to " + SAVE_FILE + ": " + t);
      t.printStackTrace();
    }
    try {
      writeToPush(totalPagesIndexed, bucket);
    } catch (IOException e) {
      System.err.println("Failed to write pending data: " + e); // Hopefully no data loss due to
                                                                // NOT_PUSHED vs INDEXED.
      e.printStackTrace();
    }
