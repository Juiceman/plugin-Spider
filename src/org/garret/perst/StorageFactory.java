...
public class StorageFactory {
  ...
  /**
   * Create new instance of the master node of replicated storage. There are two kinds of
   * replication slave nodes: statically defined and dynamically added. First one are specified by
   * replicationSlaveNodes parameter. When replication master is started it tries to eastablish
   * connection with all of the specified nodes. It is expected that state of each such node is
   * synchronized with state of the master node. It is not possible to add or remove static
   * replication slave node without stopping master node. Dynamic slave nodes can be added at any
   * moment of time. Replication master will send to such node complete snapshot of the database.
   * 
   * @param port socket port at which replication master will listen for dynamic slave nodes
   *        connections. If this parameter is -1, then no dynamic slave node conenctions are
   *        accepted.
   * @param replicationSlaveNodes addresses of static replicatin slave nodes, i.e. hosts to which
   *        replication will be performed. Address is specified as NAME:PORT
   * @param asyncBufSize if value of this parameter is greater than zero then replication will be
   *        asynchronous, done by separate thread and not blocking main application. Otherwise data
   *        is send to the slave nodes by the same thread which updates the database. If space
   *        asynchronous buffer is exhausted, then main thread will be also blocked until the data
   *        is send.
   * @return new instance of the master storage (unopened, you should explicitely invoke open
   *         method)
   */
  public ReplicationMasterStorage createReplicationMasterStorage(int port,
      String[] replicationSlaveNodes, int asyncBufSize) {
   ...
   */
  public ReplicationMasterStorage createReplicationMasterStorage(int port,
      String[] replicationSlaveNodes, int asyncBufSize, String pageTimestampFile) {
    return new ReplicationMasterStorageImpl(port, replicationSlaveNodes, asyncBufSize,
        pageTimestampFile);
  }

  /**
   * Create new instance of the static slave node of replicated storage. The address of this host
   * should be sepecified in the replicationSlaveNodes parameter of createReplicationMasterStorage
   * method. When replication master is started it tries to eastablish connection with all of the
   * specified nodes.
   * 
