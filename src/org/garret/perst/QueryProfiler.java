...
public class QueryProfiler extends StorageListener {
   ...
   * 
   * @param appendable destination where profile should be dumped (it can be StringBuilder,
   *        PrintStream,...)
   */
  public void dump(Appendable appendable) {
    ...
    for (QueryInfo info : results) {
      formatter.format(format, info.sequentialSearch ? '!' : ' ', info.totalTime, info.count,
          info.maxTime, (info.count != 0 ? info.totalTime / info.count : 0L),
          (total != 0 ? info.totalTime * 100 / total : 0L), info.query);
    }
    formatter.flush();
  }
