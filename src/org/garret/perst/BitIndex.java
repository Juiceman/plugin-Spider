...
/**
 * Interface of bit index. Bit index allows to effiicently search object with specified set of
 * properties. Each object has associated mask of 32 bites. Meaning of bits is application
 * dependent. Usually each bit stands for some binary or boolean property, for example "sex", but it
 * is possible to use group of bits to represent enumerations with more possible values.
 */
public interface BitIndex<T> extends IPersistent, IResource, ITable<T> {
   ...
   * @exception StorageError (StorageError.KEY_NOT_FOUND) exception if there is no object in the
   *            index
   */
