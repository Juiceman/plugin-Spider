...
/**
 * Interface of object spatial index. Spatial index is used to allow fast selection of spatial
 * objects belonging to the specified rectangle. Spatial index is implemented using Guttman R-Tree
 * with quadratic split algorithm.
 */
public interface SpatialIndexR2<T> extends IPersistent, IResource, ITable<T> {
   ...
   * 
   * @return minimal rectangle containing all rectangles in the index, <code>null</code> if index is
   *         empty
   */
  public RectangleR2 getWrappingRectangle();

  /**
   * Get iterator through all members of the index This iterator doesn't support remove() method. It
   * is not possible to modify spatial index during iteration.
   * 
   ...
   * @return iterator through all objects in the index in the order of increasing distance from the
   *         specified point
   */
