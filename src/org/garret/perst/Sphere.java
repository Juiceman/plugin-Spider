...
public class Sphere {
  ...
  public static class Box implements SphereObject {
    ...
    public boolean contains(double ra, double dec) {
      if ((FP.eq(dec, ne.dec) && FP.eq(dec, Math.PI / 2))
          || (FP.eq(dec, sw.dec) && FP.eq(dec, -Math.PI / 2))) {
        return true;
      }
  ...
  /**
   * A spherical ellipse is represented using two radii and a Euler transformation ( ZXZ-axis ). The
   * "untransformed" ellipse is located on equator at position (0,0). The large radius is along
   * equator.
   */
  public static class Ellipse implements SphereObject {
    ...
    public boolean equals(Object o) {
      if (o instanceof Ellipse) {
        Ellipse e = (Ellipse) o;
        return FP.eq(rad0, e.rad0) && FP.eq(rad1, e.rad1) && FP.eq(phi, e.phi)
            && FP.eq(theta, e.theta) && FP.eq(psi, e.psi);
      }
      return false;
    }
