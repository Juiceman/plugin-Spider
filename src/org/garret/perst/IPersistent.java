...
public interface IPersistent extends ILoadable, IStoreable, java.io.Externalizable {
  /**
   * Assign OID to the object. This method is used by storage class and you should not invoke it
   * directly
   * 
  ...
  /**
   * Invalidate object. Invalidated object has to be explicitly reloaded usin3g load() method.
   * Attempt to store invalidated object will cause StoraegError exception.
   */
   ...
   * "persistency on reachability apporach", but this method allows to do it explicitly
   * 
  ...
  /**
   * Specified whether object should be automatically loaded when it is referenced by other loaded
   * peristent object. Default implementation of this method returns <code>true</code> making all
   * cluster of referenced objects loaded together. To avoid main memory overflow you should stop
   * recursive loading of all objects from the database to main memory by redefining this method in
   * some classes and returing <code>false</code> in it. In this case object has to be loaded
   * explicitely using Persistent.load method.
   * 
