...
/**
 * Class representing 64-bit decimal number. It provides fixed point arithmetic and can be also used
 * in Lucene full text search engine to convert number to strings with proper lexicographical order
 * (so range operator in Lucene query can be used for numeric fields). This class supports
...
public class Decimal extends Number implements Comparable<Decimal>, IValue {
  ...
  /**
   * Constructor of decimal number from string representation. Maximal number of digits in integer
   * and fractional part of the number are calculated based on the given string value, i.e. for
   * string "00123.45" it will be 5 and 2 correspondently
   * 
