...
public class Database implements IndexProvider {
   ...
   * Database class is used. So now it is not necessary to explicitly create tables and indices -
   * the Database class will create them itself on demand. Indexable attribute should be used to
   * mark key fields for which index should be created. Table descriptor is created when instance of
   * the correspondent class is first time stored in the database. Perst creates table descriptors
   * for all derived classes up to the root java.lang.Object class.
   * 
   * @param storage opened storage. Storage should be either empty (non-initialized, either
   *        previously initialized by the this method. It is not possible to open storage with root
   *        object other than table index created by this constructor.
   * @param multithreaded <code>true</code> if database should support concurrent access to the data
   ...
   * @param helper helper for full text index classes inserted in the database
   */
  public Database(Storage storage, boolean multithreaded, boolean autoRegisterTables,
      FullTextSearchHelper helper) {
    ...
    this.globalClassExtent =
        !Boolean.FALSE.equals(storage.getProperty("perst.global.class.extent"));
    if (multithreaded) {
  ...
  /**
   * Add new record to the specified table. Record is inserted in table corresponding to the
  ...
  public boolean createIndex(Class table, String key, boolean unique) {
    return createIndex(locateTable(table, true), table, key,
        unique ? INDEX_KIND_UNIQUE : INDEX_KIND_DEFAULT);
  }
   ...
   *             database first time (to mark fields for which indices should be created, use
   *             Indexable annotaion)
   */
  @Deprecated
  ...
  @Deprecated
  public boolean createIndex(Class table, String key, boolean unique, boolean caseInsensitive,
      boolean thick, boolean randomAccess) {
    int kind = INDEX_KIND_DEFAULT;
    if (unique)
      kind |= INDEX_KIND_UNIQUE;
    if (caseInsensitive)
  ...
  private boolean createIndex(Table t, Class c, String key, int kind) {
    if (t.indicesMap.get(key) == null) {
      ...
      FieldIndex index =
          (kind & INDEX_KIND_REGEX) != 0 ? storage.createRegexIndex(c, key, caseInsensitive, 3)
   ...
   * @exception StorageError (CLASS_NOT_FOUND) exception is thrown if there is no table
   *            corresponding to the specified class
   * @return <code>true</code> if index is deleted, <code>false</code> if index is not found
   */
  ...
  /**
   * Prepare JSQL query. Prepare is needed for queries with parameters. Also preparing query can
   * improve speed if query will be executed multiple times (using prepare, it is compiled only
   * once).
   * <P>
   ...
   * @return result of query execution ordered by rank (of sort==true) or null in case of empty or
   *         incorrect query
   */
