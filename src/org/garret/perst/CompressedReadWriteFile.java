...
/**
 * Compressed read-write database file. To work with compressed database file you should pass
...
public class CompressedReadWriteFile implements IFile {
   ...
   * @param pageIndexCheckpointThreshold maximal size of page index log file, after reaching this
   *        size page index is flushed and log is truncsated
   ...
   * @param cipherKey cipher key (if null, then no encryption is performed)
   */
  public CompressedReadWriteFile(String dataFilePath, String pageIndexFilePath,
      String pageIndexLogFilePath, long dataFileExtensionQuantum, long pageIndexCheckpointThreshold,
      long pageIndexInitSize, boolean readOnly, boolean noFlush, String cipherKey) {
    ...
    try {
      ...
      pageIndexBuffer = pageIndexChan.map(
          readOnly ? FileChannel.MapMode.READ_ONLY : FileChannel.MapMode.READ_WRITE, 0, // position
          pageIndexSize);
      ...
      if (!readOnly) {
        ...
        while ((size -= 8) >= 0) {
          ...
          long pageBitSize =
              ((pagePos & (Page.pageSize - 1)) + ALLOCATION_QUANTUM) >>> ALLOCATION_QUANTUM_LOG;
  ...
  long allocate(int bitSize) {
    ...
    if (pos < 0) {
      pos = BitmapAllocator.allocate(bitmap, bitmapStart,
          BitmapAllocator.locateHoleEnd(bitmap, bitmapPos), bitSize);
      if (pos < 0) {
        byte[] newBitmap = new byte[bitmap.length + bitmapExtensionQuantum];
        System.arraycopy(bitmap, 0, newBitmap, 0, bitmap.length);
        pos = BitmapAllocator.allocate(newBitmap,
            BitmapAllocator.locateBitmapEnd(newBitmap, bitmap.length), newBitmap.length, bitSize);
        Assert.that(pos >= 0);
        bitmap = newBitmap;
      }
    }
