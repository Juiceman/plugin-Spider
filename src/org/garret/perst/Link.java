...
/**
 * Interface for one-to-many relation. There are two types of relations: embedded (when references
 * to the relarted obejcts are stored in relation owner obejct itself) and stanalone (when relation
 * is separate object, which contains the reference to the relation owner and relation members).
 * Both kinds of relations implements Link interface. Embedded relation is created by
 * Storage.createLink method and standalone relation is represented by Relation persistent class
 * created by Storage.createRelation method.
 */
public interface Link<T> extends ITable<T>, List<T>, RandomAccess {
  ...
  /**
   * Get related object by index without loading it. Returned object can be used only to get it OID
   * or to compare with other objects using <code>equals</code> method
   * 
   * @param i index of the object in the relation
  ...
  /**
   * Assign value to i-th element of the relation. Unlike Link.set methos this method doesn't return
   * previous value of the element and so is faster if previous element value is not needed (it has
   * not to be fetched from the database)
   * 
   * @param i index in the relartion
   * @param obj object to be included in the relation
   */
   ...
   * specified array. If the index fits in the specified array, it is returned therein. Otherwise, a
   * new array is allocated with the runtime type of the specified array and the size of this index.
   * <p>
   *
   * If this index fits in the specified array with room to spare (i.e., the array has more elements
   * than this index), the element in the array immediately following the end of the index is set to
   * <tt>null</tt>. This is useful in determining the length of this index <i>only</i> if the caller
   * knows that this index does not contain any <tt>null</tt> elements.)
   * <p>
   * 
