...
/**
 * Listener of database events. Programmer should derive his own subclass and register it using
 * Storage.setListener method.
 */
public abstract class StorageListener {
  /**
   * This metod is called during database open when database was not close normally and has to be
   * recovered
   */
  ...
  /**
   * This method is called when garbage collection is started (ether explicitly by invocation of
   * Storage.gc() method, either implicitly after allocation of some amount of memory)).
   */
  public void gcStarted() {}

  /**
   * This method is called when unreferenced object is deallocated from database during garbage
   * collection. It is possible to get instance of the object using
  ...
  /**
   * This method is called by XMLExporter when exception happens during object export. Exception can
   * be caused either by IO problems (failed to write data to the destination stream) either by
   * corruption of object.
   * 
   * @param oid object identifier of exported object
   * @param x catched exception
   * @return <b>true</b> if error should be ignored and export continued, <b>false</b> to rethrow
   *         catched exception
   */
   ...
   * 
   * @param host address of host replication to which is failed (null if error jappens at slave
   *        node)
   ...
   * @param elapsedTime time in milliseconds of query execution (please notice that many queries are
   *        executed incrementally, so the time passed to this method is not always complete time of
   *        query execution
   * @param sequentialSearch true if sequential scan in used for query execution, false if some
   *        index is used
   */
  public void queryExecution(Class table, String query, long elapsedTime,
  ...
  /**
   * This method is called by XML exporter if database correuption or some other reasons makes
   * export of the object not possible
   * 
   * @param oid object identified
   * @param x catched exception
   * @return <b>true</b> to ignore error and continue export, <b>false</b> to cancel export and
   *         rethrow exception
   */
  ...
  /**
   * Index is automaticaslly created by Database class when query is executed and autoIndices is
   * anabled
   * 
