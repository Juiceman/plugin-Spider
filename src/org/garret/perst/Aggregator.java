...
/**
 * Class used to perform aggregation. Grouping and aggregation is done using two interfaces
 * <code>Aggregate</code> and <code>GroupBy</code>. <code>Aggregate</code> interface is used to
 * provide aggregate implementations. There are some predefined implementations for standard
 * aggregates: max, min, sum, avg, count... And it is possible to define own aggregates.
 * <p>
 * <code>GroupBy</code> interface should be implemented by programmer (it is convenient to use
 * anonymous classes) and defines aggregation query: how to split input data into groups and which
 * aggregates to calculate for the group. So this class gives answer for the questions:
 * <ol>
 ...
 * </ol>
 * Aggregator is using map to associate aggregate states with groups. This map is returned as result
 * of aggregation. Aggregator can use ordered or unordered map (TreeMap or HashMap). Ordered map
 * returns results in ascending order of group-by values.
 * <p>
 * Example:
 * 
 * <pre>
 ...
 *   }
 * };
 * // Query: select standard deviation of difference between low and high prices for IBM for each
 * // month during 1990 till 2012
 * Stock stock = stocks.get("IBM");
 * IterableIterator&lt;Quote&gt; iterator =
 *     stock.quotes.iterator(new Date(1990, 0, 1), new Date(2012, 0, 1));
 * Map&lt;Object, Aggregator.Aggregate&gt; result =
 *     Aggregator.&lt;Quote&gt;aggregate(iterator, new Aggregator.GroupBy&lt;Quote&gt;() {
...
public class Aggregator {
  /**
   * Aggregate interface. It is implementye by all standard aggregates and can be use to define own
   * (custom) aggregates
   */
  public interface Aggregate<T> {
    ...
    /**
     * Merge two aggregate states. This method is used by Aggregator.merge method by also can be use
     * explicitly to combine two aggregation results.
     * 
  ...
  public interface GroupBy<T> {
     ...
     * Group-by field(s).
     * 
     * @return For single field it is ebough to return wrapper object for the field value. For
     *         multiple fields you should construct class and implement Object.hashCode(),
     *         Object.equals() and optionally Comparable.compareTo() methods
     */
    Object getKey(T obj);
   ...
   * @param iterable collection of aggregated objects
   * @param groupBy aggregation operation
   * @param orderByKey specify whether ordered map (TreeMap) should be used for grouping. Group-by
   *        kley should privide comparison operation in this case.
   * @return map with results of aggregation: &lt;group-by,aggregate-value&gt; pairs
   */
  public static <T> Map<Object, Aggregate> aggregate(Iterable<T> iterable, GroupBy<T> groupBy,
      boolean orderByKey) {
    Iterator<T> iterator = iterable.iterator();
    Map<Object, Aggregate> hash =
        orderByKey ? (Map<Object, Aggregate>) new TreeMap<Object, Aggregate>()
  ...
  /**
   * Merge two aggregation results. This method combines state of aggregates in "dst" with aggregate
   * states in "src", i.e. dst = merge(dst, src)
   */
  ...
  public static class TopAggregate implements Aggregate<Comparable> {
     ...
     * 
     * @return array with top N values. If there are less than N object in aggregated set, then size
     *         of result array may be smaller than N
     */
    @Override
  ...
  /**
   * Compound aggregate: combination of several aggregates allowsing to caclulate more than one
   * aggregate at one traversal
   */
