...
public class ReplicationMasterFile implements IFile, Runnable {
   ...
   * @param hosts slave node hosts to which replicastion will be performed
   * @param ack whether master should wait acknowledgment from slave node during trasanction commit
   * @param pageTimestampFilePath path to the file with pages timestamps. This file is used for
   *        synchronizing with master content of newly attached node
   */
  ...
  private ReplicationMasterFile(ReplicationMasterStorageImpl storage, IFile file, int port,
    ...
    if (pageTimestampFilePath != null) {
      pageTimestampFile =
          new OSFile(pageTimestampFilePath, false, storage != null && storage.noFlush);
      ...
      dirtyPageTimestampMap =
          new int[(((pageTimestamps.length * 4 + Page.pageSize - 1) >> Page.pageSizeLog)
              + 31) >> 5];
    ...
    if (port >= 0) {
      storage.setProperty("perst.alternative.btree", Boolean.TRUE); // prevent direct modification
                                                                    // of pages
      try {
      ...
      Sync: do {
        ...
        for (long pos = 0; pos < size; pos += Page.pageSize) {
          ...
          if (syncNodeTimestamps != null) {
            if (pageNo < syncNodeTimestamps.length && pageNo < pageTimestamps.length
          ...
          synchronized (storage) {
            synchronized (storage.objectCache) {
              ...
              if (syncNodeTimestamps != null) {
                Bytes.pack4(txBuf, Page.pageSize + 8,
                    pageNo < pageTimestamps.length ? pageTimestamps[pageNo] : 0);
              }
            }
          }
          Page pg = storage.pool.getPage(pos);
    ...
    synchronized (mutex) {
      if (pageTimestamps != null) {
        ...
        if (pageNo >= pageTimestamps.length) {
          int newLength =
              pageNo >= pageTimestamps.length * 2 ? pageNo + 1 : pageTimestamps.length * 2;
          ...
          int[] newDirtyPageTimestampMap =
              new int[(((newLength * 4 + Page.pageSize - 1) >> Page.pageSizeLog) + 31) >> 5];
        ...
        dirtyPageTimestampMap[pageNo >> (Page.pageSizeLog - 2 + 5)] |=
            1 << ((pageNo >> (Page.pageSizeLog - 2)) & 31);
      }
