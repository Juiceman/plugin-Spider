...
public class XMLImporter {
        ...
        switch (types[i]) {
          ...
          case ClassDescriptor.tpArrayOfByte:
            ...
            for (int j = 0, n = value.length(); j < n; j += 2) {
              buf.arr[dst++] =
                  (byte) ((getHexValue(value.charAt(j)) << 4) | getHexValue(value.charAt(j + 1)));
            }
            break;
          default:
  ...
  final void createIndex(String indexType) throws XMLImportException {
    ...
    if (tkn != XMLScanner.XML_LTS || scanner.scan() != XMLScanner.XML_IDENT
        || !scanner.getIdentifier().equals(indexType) || scanner.scan() != XMLScanner.XML_GT) {
  ...
  final int importBinary(XMLElement elem, int offs, ByteBuffer buf, String fieldName)
    ...
    if (elem == null || elem.isNullValue()) {
      ...
      for (int j = 0; j < len; j += 2) {
        buf.arr[offs++] =
            (byte) ((getHexValue(hexStr.charAt(j)) << 4) | getHexValue(hexStr.charAt(j + 1)));
      }
    } else {
        ...
        case ClassDescriptor.tpEnum:
          ...
          if (elem != null) {
            if (elem.isIntValue()) {
              ...
              Bytes.pack4(buf.arr, offs,
                  Enum.valueOf((Class) fd.field.getType(), elem.getStringValue()).ordinal());
            } else {
              throwException("Conversion for field " + fieldName + " is not possible");
            }
          }
    ...
    while (true) {
      switch (scanner.scan()) {
        ...
        case XMLScanner.XML_GT:
          ...
          if (tkn != XMLScanner.XML_LTS || scanner.scan() != XMLScanner.XML_IDENT
              || !scanner.getIdentifier().equals(name) || scanner.scan() != XMLScanner.XML_GT) {
            throwException("Element is not closed");
          }
          return elem;
        case XMLScanner.XML_IDENT:
