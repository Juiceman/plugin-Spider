...
import org.garret.perst.PointRn;
  ...
  class NeighborIterator<E> extends IterableIterator<E> implements PersistentIterator {
    ...
    public boolean hasNext() {
      ...
      while (true) {
        ...
        list = neighbor.next;
        RtreeRnPage pg = (RtreeRnPage) (neighbor.child instanceof RtreeRnPage ? neighbor.child
            : storage.getObjectByOID(storage.getOid(neighbor.child)));
        for (int i = 0, n = pg.n; i < n; i++) {
    ...
    public E next() {
      ...
      return (E) (neighbor.child instanceof PersistentStub
          ? storage.getObjectByOID(storage.getOid(neighbor.child))
          : neighbor.child);
    }
