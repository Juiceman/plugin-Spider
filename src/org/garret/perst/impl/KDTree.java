    ...
    private boolean getMin(KDTreeNode<T> node) {
      if (node != null) {
        while (true) {
          node.load();
          stack.push(node);
          int diff = low == null ? MultidimensionalComparator.LEFT_UNDEFINED
    ...
    public boolean hasNext() {
      ...
      while (!stack.empty()) {
        ...
        if (node != null) {
          ...
          if (node.right != null && (high == null || comparator.compare(high, node.obj,
              stack.size() % nDims) != MultidimensionalComparator.LT)) {
            stack.push(null);
