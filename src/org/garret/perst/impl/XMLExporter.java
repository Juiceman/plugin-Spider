...
public class XMLExporter {
  ...
  public void exportDatabase(int rootOid) throws IOException {
    ...
    writer.write("<database root=\"" + rootOid + "\" maxoids=\"" + storage.currIndexSize + "\">\n");
              ...
              try {
                ...
                if (desc.cls == Btree.class) {
                  ...
                  exportFieldIndex(oid, obj,
                      "org.garret.perst.impl.BtreeCaseInsensitiveFieldIndex");
                  ...
                  exportMultiFieldIndex(oid, obj,
                      "org.garret.perst.impl.BtreeCaseInsensitiveMultiFieldIndex");
  ...
  final int exportKey(byte[] body, int offs, int size, int type) throws IOException {
    switch (type) {
      ...
      case ClassDescriptor.tpDate: {
        ...
        offs += 8;
        if (msec != Storage.INVALID_DATE) {
          writer.write(httpFormatter.format(new Date(msec)));
        } else {
          writer.write("null");
        }
        break;
      ...
      switch (fd.type) {
        ...
        case ClassDescriptor.tpEnum: {
          ...
          if (ordinal < 0) {
            ...
            writer.write(
                "\"" + ((Enum) fd.field.getType().getEnumConstants()[ordinal]).name() + "\"");
          }
          offs += 4;
          break;
        }
        ...
        case ClassDescriptor.tpArrayOfFloat: {
          ...
          if (len < 0) {
            ...
            while (--len >= 0) {
              indentation(indent + 1);
              writer.write(
                  "<element>" + Float.intBitsToFloat(Bytes.unpack4(body, offs)) + "</element>\n");
              offs += 4;
            }
            indentation(indent);
          }
