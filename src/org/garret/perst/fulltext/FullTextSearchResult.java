...
public class FullTextSearchResult {
  /**
   * Estimation of total number of documents in the index matching this query. Full text search
   * query result is usually limited by number of returned documents and query execution time. So
   * there are can be more documents in the index matching this query than actually returned. This
   * field provides estimation for total number of documents matching the query.
   */
  ...
  public FullTextSearchResult merge(FullTextSearchResult another) {
    ...
    for (int i = 1; i < joinHits.length; i++) {
      if (joinHits[i].oid == joinHits[i - 1].oid) {
        joinHits[n++] = new FullTextSearchHit(joinHits[i].storage, joinHits[i].oid,
            joinHits[i - 1].rank + joinHits[i].rank);
        i += 1;
      }
    }
