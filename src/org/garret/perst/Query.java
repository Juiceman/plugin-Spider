...
/**
 * Class representing JSQL query. JSQL allows to select members of Perst collections using SQL like
 * predicate. Almost all Perst collections have select() method which execute arbitrary JSQL query.
 * But it is also possible to create Query instance explicitely, Using storage.createQuery class. In
 * this case it is possible to specify query with parameters, once prepare query and then multiple
 * times specify parameters and execute it. Also Query interface allows to specify <i>indices</i>
 * and <i>resolvers</i>. JSQL can use arbitrary Perst <code>GenericIndex</code> to perform fast
 * selection if object instead of sequeial search. And user provided <i>resolver</i> allows to
