...
public interface Storage {
  /**
   * Constant specifying that page pool should be dynamically extended to conatins all database file
   * pages
   */
   ...
   * cooperative and serializable. In case of exclusive transaction, only one thread can update the
   * database. In cooperative mode, multiple transaction can work concurrently and commit() method
   * will be invoked only when transactions of all threads are terminated. Serializable transactions
   * can also work concurrently. But unlike cooperative transaction, the threads are isolated from
   * each other. Each thread has its own associated set of modified objects and committing the
   * transaction will cause saving only of these objects to the database. To synchronize access to
   * the objects in case of serializable transaction programmer should use lock methods of IResource
   * interface. Shared lock should be set before read access to any object, and exclusive lock -
   * before write access. Locks will be automatically released when transaction is committed (so
   * programmer should not explicitly invoke unlock method) In this case it is guaranteed that
   * transactions are serializable.<br>
   * It is not possible to use <code>IPersistent.store()</code> method in serializable transactions.
   * That is why it is also not possible to use Index and FieldIndex containers (since them are
   * based on B-Tree and B-Tree directly access database pages and use <code>store()</code> method
   * to assign OID to inserted object. You should use <code>SortedCollection</code> based on T-Tree
   * instead or alternative B-Tree implemenataion (set "perst.alternative.btree" property).
   * 
   ...
   */
  public <T> FieldIndex<T> createFieldIndex(Class type, String fieldName, boolean unique,
      boolean caseInsensitive, boolean thick);
   ...
   * @param maxBlockTimeInterval maximal difference in milliseconds between timestamps of the first
   *        and the last elements in a block. If value of this parameter is too small, then most
   *        blocks will contains less elements than preallocated. If it is too large, then searching
   *        of block will be inefficient, because index search will select a lot of extra blocks
   *        which do not contain any element from the specified range. Usually the value of this
   *        parameter should be set as (number of elements in block)*(tick interval)*2. Coefficient
   *        2 here is used to compencate possible holes in time series. For example, if we collect
   *        stocks data, we will have data only for working hours. If number of element in block is
   *        100, time series period is 1 day, then value of maxBlockTimeInterval can be set as
   *        100*(24*60*60*1000)*2
   * @return new empty time series
   */
  public <T extends TimeSeries.Tick> TimeSeries<T> createTimeSeries(Class blockClass,
   ...
   * its OID can be reused. In this case getObjectByOID will return reference to the new object with
   ...
   * following boolean properties are supported:
   * <TABLE summary="">
   * <TR>
   * <TH>Property name</TH>
   ...
   * </TR>
   * <TR>
   * <TD><code>perst.implicit.values</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Treate any class not derived from IPersistent as <i>value</i>. This object will be embedded
   * inside persistent object containing reference to this object. If this object is referenced from
   * N persistent object, N instances of this object will be stored in the database and after
   * loading there will be N instances in memory. As well as persistent capable classes, value
   * classes should have default constructor (constructor with empty list of parameters) or has no
   * constructors at all. For example <code>Integer</code> class can not be stored as value in PERST
   * because it has no such constructor. In this case serialization mechanism can be used (see
   * below)</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.serialize.transient.objects</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Serialize any class not derived from IPersistent or IValue using standard Java
   * serialization mechanism. Packed object closure is stored in database as byte array. Latter the
   * same mechanism is used to unpack the objects. To be able to use this mechanism object and all
   * objects referenced from it should implement <code>java.io.Serializable</code> interface and
   * should not contain references to persistent objects. If such object is referenced from N
   * persistent object, N instances of this object will be stored in the database and after loading
   * there will be N instances in memory.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.object.cache.init.size</code></TD>
   * <TD>Integer</TD>
   * <TD>1319</TD>
   * <TD>Initial size of object cache</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.object.cache.kind</code></TD>
   * <TD>String</TD>
   * <TD>"lru"</TD>
   * <TD>Kind of object cache. The following values are supported: "strong", "weak", "soft",
   * "pinned", "lru". <B>Strong</B> cache uses strong (normal) references to refer persistent
   * objects. Thus none of loaded persistent objects can be deallocated by GC. <B>Weak</B> cache
   * usea weak references and soft cache - <B>soft</B> references. The main difference between soft
   * and weak references is that garbage collector is not required to remove soft referenced objects
   * immediately when object is detected to be <i>soft referenced</i>, so it may improve caching of
   * objects. But it also may increase amount of memory used by application, and as far as
   * persistent object requires finalization it can cause memory overflow even though garbage
   * collector is required to clear all soft references before throwing OutOfMemoryException.<br>
   * But Java specification says nothing about the policy used by GC for soft references (except the
   * rule mentioned above). Unlike it <B>lru</B> cache provide determined behavior, pinning most
   * recently used objects in memory. Number of pinned objects is determined for lru cache by
   * <code>perst.object.index.init.size</code> parameter (it can be 0).<br>
   * Pinned object cache pin in memory all modified objects while using weak referenced for
   * non-modified objects. This kind of cache eliminate need in finalization mechanism - all
   * modified objects are kept in memory and are flushed to the disk only at the end of transaction.
   * So the size of transaction is limited by amount of main memory. Non-modified objects are
   * accessed only through weak references so them are not protected from GC and can be thrown away.
   * </TD>
   * </TR>
   * <TR>
   * <TD><code>perst.object.index.init.size</code></TD>
   * <TD>Integer</TD>
   * <TD>1024</TD>
   * <TD>Initial size of object index (specifying large value increase initial size of database, but
   * reduce number of index reallocations)</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.extension.quantum</code></TD>
   * <TD>Long</TD>
   * <TD>1048576</TD>
   * <TD>Object allocation bitmap extension quantum. Memory is allocate by scanning bitmap. If there
   * is no large enough hole, then database is extended by the value of dbDefaultExtensionQuantum.
   * This parameter should not be smaller than 64Kb.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.gc.threshold</code></TD>
   * <TD>Long</TD>
   * <TD>Long.MAX_VALUE</TD>
   * <TD>Threshold for initiation of garbage collection. If it is set to the value different from
   * Long.MAX_VALUE, GC will be started each time when delta between total size of allocated and
   * deallocated objects exceeds specified threashold OR after reaching end of allocation bitmap in
   * allocator.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.lock.file</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Lock database file to prevent concurrent access to the database by more than one
   * application.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.file.readonly</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Database file should be opened in read-only mode.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.file.noflush</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Do not flush file during transaction commit. It will greatly increase performance because
   * eliminate synchronous write to the disk (when program has to wait until all changed are
   * actually written to the disk). But it can cause database corruption in case of OS or power
   * failure (but abnormal termination of application itself should not cause the problem, because
   * all data which were written to the file, but is not yet saved to the disk is stored in OS file
   * buffers and sooner or later them will be written to the disk)</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.alternative.btree</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Use aternative implementation of B-Tree (not using direct access to database file pages).
   * This implementation should be used in case of serialized per thread transctions. New
   * implementation of B-Tree will be used instead of old implementation if
   * "perst.alternative.btree" property is set. New B-Tree has incompatible format with old B-Tree,
   * so you could not use old database or XML export file with new indices. Alternative B-Tree is
   * needed to provide serializable transaction (old one could not be used). Also it provides better
   * performance (about 3 times comaring with old implementation) because of object caching. And
   * B-Tree supports keys of user defined types.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.background.gc</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Perform garbage collection in separate thread without blocking the main application.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.string.encoding</code></TD>
   * <TD>String</TD>
   * <TD>null</TD>
   * <TD>Specifies encoding of storing strings in the database. By default Perst stores strings as
   * sequence of chars (two bytes per char). If all strings in application are in the same language,
   * then using encoding can signifficantly reduce space needed to store string (about two times).
   * But please notice, that this option has influence on all strings stored in database. So if you
   * already have some data in the storage and then change encoding, then it will cause database
   * crash.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.replication.ack</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Request acknowledgement from slave that it receives all data before transaction commit. If
   * this option is not set, then replication master node just writes data to the socket not warring
   * whether it reaches slave node or not. When this option is set to true, master not will wait
   * during each transaction commit acknowledgement from slave node. Please notice that this option
   * should be either set or not set both at slave and master node. If it is set only on one of this
   * nodes then behavior of the system is unpredicted. This option can be used both in synchronous
   * and asynchronous replication mode. The only difference is that in first case main application
   * thread will be blocked waiting for acknowledgment, while in the asynchronous mode special
   * replication thread will be blocked allowing thread performing commit to proceed.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.concurrent.iterator</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>By default iterator will throw ConcurrentModificationException if iterated collection was
   * changed outside iterator, when the value of this property is true then iterator will try to
   * restore current position and continue iteration</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.slave.connection.timeout</code></TD>
   * <TD>Integer</TD>
   * <TD>60</TD>
   * <TD>Timeout in seconds during which mastr node will try to establish connection with slave
   * node. If connection can not be established within specified time, then master will not perform
   * replication to this slave node</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.force.store</code></TD>
   * <TD>Boolean</TD>
   * <TD>true</TD>
   * <TD>When the value of this parameter is true Storage.makePersistent method cause immediate
   * storing of object in the storage, otherwise object is assigned OID and is marked as modified.
   * Storage.makePersistent method is mostly used when object is inserted in B-Tree. If application
   * put in index object referencing a large number of other objects which also has to be made
   * persistent, then marking object as modified instead of immediate storing may cause memory
   * overflow because garbage collector and finalization threads will store objects with less speed
   * than application creates new ones. But if object will be updated after been placed in B-Tree,
   * then immediate store will just cause cause extra overhead, because object has to be stored
   * twice.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.page.pool.lru.limit</code></TD>
   * <TD>Long</TD>
   * <TD>1L &lt;&lt; 60</TD>
   * <TD>Set boundary for caching database pages in page pool. By default Perst is using LRU
   * algorithm for finding candidate for replacement. But for example for BLOBs this strategy is not
   * optimal and fetching BLOB can cause flushing the whole page pool if LRU discipline is used. And
   * with high probability fetched BLOB pages will no be used any more. So it is preferable not to
   * cache BLOB pages at all (throw away such page immediately when it is not used any more). This
   * parameter in conjunction with custom allocator allows to disable caching for BLOB objects. If
   * you set value of "perst.page.lru.scope" property equal to base address of custom allocator
   * (which will be used to allocate BLOBs), then page containing objects allocated by this
   * allocator will not be cached in page pool.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.multiclient.support</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>Supports access to the same database file by multiple applications. In this case Perst will
   * use file locking to synchronize access to the database file. An application MUST wrap any
   * access to the database with beginThreadThreansaction/endThreadTransaction methods. For read
   * only access use READ_ONLY_TRANSACTION mode and if transaction may modify database then
   * READ_WRITE_TRANSACTION mode should be used.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.reload.objects.on.rollback</code></TD>
   * <TD>Boolean</TD>
   * <TD>false</TD>
   * <TD>By default, Perst doesn't reload modified objects after a transaction rollback. In this
   * case, the programmer should not use references to the persistent objects stored in program
   * variables. Instead, the application should fetch the object tree from the beginning, starting
   * from obtaining the root object using the Storage.getRoot method.
   *
   * Setting the "perst.reload.objects.on.rollback" property instructs Perst to reload all objects
   * modified by the aborted (rolled back) transaction. It takes additional processing time, but in
   * this case it is not necessary to ignore references stored in variables, unless they point to
   * the objects created by this transactions (which were invalidated when the transaction was
   * rolled back). Unfortunately, there is no way to prohibit access to such objects or somehow
   * invalidate references to them. So this option should be used with care.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.reuse.oid</code></TD>
   * <TD>bool</TD>
   * <TD>true</TD>
   * <TD>This parameters allows to disable reusing OID of deallocated objects. It can simplify
   * debugging of application performing explicit deallocation of objects (not using garbage
   * collection). Explicit object deallocation can cause "dangling references" problem - when there
   * are live references to the deallocated object. Access to such object should cause
   * <code>StorageError(DELETED_OBJECT)</code> exception. But if OID of the object can be reused and
   * assigned to some newly deallocated object, then we will get type cast or field access errors
   * when try to work with this object. In the worst case OID will be reused by the object of the
   * same type - then application will not notice that referenced object was substituted. Disabling
   * reuse of OID allows to eliminate such unpredictable behavior - access to the deallocated object
   * will always cause <code>StorageError(DELETED_OBJECT)</code> exception. But please notice that
   * disabling reuse of OID for a long time and intensive allocation/deallocation of objects can
   * cause exhaustion of OID space (2Gb).</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.compatibilit.mode</code></TD>
   * <TD>int</TD>
   * <TD>0</TD>
   * <TD>Bitmask of different compatibility modes. Right now the following modes are supported:
   * IBM_JAVA5_COMPATIBILITY_MODE: compatibility with databases created by Perst prior to 2.73
   * release by IBM Java5 VM CLASS_LOADER_SERIALIZATION_COMPATIBILITY_MODE: compatibility with
   * databases created by Perst prior to 2.66 and using custom class loaders and serialization of
   * transient objects</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.global.class.extent</code></TD>
   * <TD>bool</TD>
   * <TD>true</TD>
   * <TD>This parameter is used by Database class in "auto register table" mode. In this mode Perst
   * automatically creates indices (class extents) for all derived classes of the inserted object if
   * there are no such class extents yet. It include class extent for Perstistent class allowing to
   * enumerate all objects in the storage. If such list is not needed, then this option can be set
   * to false to eliminate extra index maintenance overhead.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.search.base.classes</code></TD>
   * <TD>bool</TD>
   * <TD>true</TD>
   * <TD>This parameter is used by Database class. If there is no table (class extent) corresponding
   * to the requested class, then Perst tries to locate class extend for base class and so on. By
   * setting this property to false it is possible to prohibit lookup of base classes. Please notice
   * that lookup on base classes is also not performed if "auto register table" mode is active.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.xml.date.format</code></TD>
   * <TD>String</TD>
   * <TD>"EEE, d MMM yyyy kk:mm:ss z"</TD>
   * <TD>Date format used for XML export/import. By default HTTP date format is used.</TD>
   * </TR>
   * <TR>
   * <TD><code>perst.xml.reuse.oid</code></TD>
   * <TD>bool</TD>
   * <TD>false</TD>
   * <TD>Reuse OID of objects during XML import</TD>
   * </TR>
   * </TABLE>
   * 
   * @param name name of the property
   * @param value value of the property (for boolean properties pass
   *        <code>java.lang.Boolean.TRUE</code> and <code>java.lang.Boolean.FALSE</code>
   */
   ...
   * be used to control recursive loading.
   * 
   * @param type Class for which recursive loading policy is specified. By default recursive loading
   *        is enabled for all classes. Disabling recursive loading for some class also affect all
   *        derived classes unless policy is explicitly specified for such class.
