...
/**
 * Interface of indexed field. Index is used to provide fast access to the object by the value of
 * indexed field. Objects in the index are stored ordered by the value of indexed field. It is
 * possible to select object using exact value of the key or select set of objects which key belongs
 * to the specified interval (each boundary can be specified or unspecified and can be inclusive or
 * exclusive) Key should be of scalar, String, java.util.Date or peristent object type.
 */
public interface FieldIndex<T> extends GenericIndex<T> {
   ...
   *        (<code>int</code> or <code>long</code>) type. This field is assigned unique value (which
   *        will not be reused while this index exists) and object is marked as modified. Object can
   *        be not yet peristent, in this case its forced to become persistent by assigning OID to
   *        it.
   * @exception StorageError (StorageError.INCOMPATIBLE_KEY_TYPE) when indexed field has type other
