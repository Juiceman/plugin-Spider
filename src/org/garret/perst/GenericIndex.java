...
/**
 * Interface of object index. This is base interface for Index and FieldIndex, allowing to write
 * generic algorithms working with both itype of indices.
 */
public interface GenericIndex<T> extends IPersistent, IResource, ITable<T> {
  ...
  /**
   * Get iterator for traversing all entries in the index. Iterator next() method returns object
   * implementing <code>Map.Entry</code> interface which allows to get entry key and value. Objects
   * are iterated in the ascent key order. This iterator supports remove() method. To make it
   * possible to update, remove or add members to the index during iteration it is necessary to set
   * "perst.concurrent.iterator" property (by default it is not supported because it cause extra
  ...
  /**
   * Get entry iterator of objects in the index starting with specified position. This methid is
   * efficient only for random access indices You should not update/remove or add members to the
  ...
  /**
   * Get iterator for traversing index entries with key belonging to the specified range. Iterator
   * next() method returns object implementing <code>Map.Entry</code> interface This iterator
   * supports remove() method. To make it possible to update, remove or add members to the index
   * during iteration it is necessary to set "perst.concurrent.iterator" property (by default it is
   * not supported because it cause extra overhead during iteration)
   * 
   * @param from low boundary. If <code>null</code> then low boundary is not specified. Low boundary
   *        can be inclusive or exclusive.
   * @param till high boundary. If <code>null</code> then high boundary is not specified. High
   *        boundary can be inclusive or exclusive.
   * @param order <code>ASCENT_ORDER</code> or <code>DESCENT_ORDER</code>
   ...
   * @exception StorageError (StorageError.KEY_NOT_UNIQUE) exception if there are more than one
   *            objects in the index with specified value of the key.
   */
  public T get(Key key);

  /**
   * Get objects which key value belongs to the specified range. Either from boundary, either till
   * boundary either both of them can be <code>null</code>. In last case the method returns all
   ...
   * @exception IndexOutOfBoundsException if position is less than 0 or greater or equal than index
   *            size
   */
  T getAt(int i);
  ...
  /**
   * Get objects with objects with key started with specified prefix, i.e. getPrefix("abc") will
   * return "abc", "abcd", "abcdef", ... but not "ab".
   * 
  ...
  /**
   * Get position of the first element with specified key. This method is efficient only for random
   * access indices
   * 
   ...
   * order. This iterator supports remove() method. To make it possible to update, remove or add
   ...
   * This iterator supports remove() method. To make it possible to update, remove or add members to
   * the index during iteration it is necessary to set "perst.concurrent.iterator" property (by
   * default it is not supported because it cause extra overhead during iteration)
   * 
   * @param from low boundary. If <code>null</code> then low boundary is not specified. Low boundary
   *        can be inclusive or exclusive.
   * @param till high boundary. If <code>null</code> then high boundary is not specified. High
  ...
  /**
   * Get iterator for records which keys started with specified prefix Objects are iterated in the
   * ascent key order. This iterator supports remove() method. To make it possible to update, remove
   * or add members to the index during iteration it is necessary to set "perst.concurrent.iterator"
   * property (by default it is not supported because it cause extra overhead during iteration)
   * 
  ...
  /**
   * Get ascent or descent iterator for records which keys started with specified prefix Objects are
   * iterated in the ascent key order. This iterator supports remove() method. To make it possible
   * to update, remove or add members to the index during iteration it is necessary to set
   * "perst.concurrent.iterator" property (by default it is not supported because it cause extra
   * overhead during iteration)
   * 
   * @param prefix key prefix
  ...
  /**
   * Locate all objects which key is prefix of specified word, i.e. prefixSearch("12345") will
   * return "12", "123", "1234", "12345", but not "123456"
   * 
